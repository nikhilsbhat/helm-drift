...
func (drift *Drift) getChartFromTemplate() ([]byte, error) {
	...
	drift.log.Debug(fmt.Sprintf("rendering helm chart with following commands/flags '%s'", strings.Join(args, ", ")))
	...
	if errors.As(err, &exitErr) {
		drift.log.Error(fmt.Sprintf("rendering template for release: '%s' errored with ", drift.release), err)
...
func (drift *Drift) getTemplates(template []byte) []string {
	drift.log.Debug(fmt.Sprintf("splitting helm manifests with regex pattern: '%s'", drift.Regex))
